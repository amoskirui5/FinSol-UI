...
import { FinancialYear, FinancialYearApiPayload, FinancialYearListResponse } from "../types/Settings/financialYearTypes";
...
export const updateFinancialYear = async (financialYear: FinancialYearApiPayload): Promise<FinancialYear> => {
  const response = await axiosInstance.put(`${UPDATE_FINANCIAL_YEAR}/${financialYear.financialYearId}`, financialYear);
  return response.data;
}
